...
def run_ltspice_meas_out(simname, **kwargs):
    ...
    if os.path.isfile('param.txt') and filecmp.cmp('param.txt_', 'param.txt'):
        logging.warning("param.txt has not changed")
    else:
        sth_changed = True
    ...
    if sth_changed:
        ...
        logging.warning("simulation input data did not change.")
    ...
    with open(outfile, "r") as outf:
        with open(outfile_cleaned, "w") as cleanf:
            ...
            cleanf.close()
    if fail:
        logging.error("simulation of {:s}.asc recorded FAIL in output".format(simname))
